...
void initTracker(Mat image, int winWidth, int winHeight, Rect rect) {
	...
	inRange(obj.hsv, Scalar(0, smin, MIN(_vmin, _vmax)), Scalar(180, 256, MAX(_vmin, _vmax)), obj.mask);
	int ch[] = { 0, 0 };
...
void trackFace(TrackObj& trackObj, Mat image, float *result, int imageWidth, int imageHeight, JNIEnv *env, jobject obj) {
...
//			trackObj.trackFaceRect.height);
	...
	Rect finalRect = Rect(trackObj.trackFaceRect.x, trackObj.trackFaceRect.y,trackObj.trackFaceRect.height, trackObj.trackFaceRect.height);
	int scaleSize = finalRect.width - (int) ((float) finalRect.width / 5.0 * 4);
	finalRect = Rect(finalRect.x, finalRect.y + scaleSize / 2,finalRect.width / 2, finalRect.height - scaleSize);
...
int detectFace(Mat image, float *result, int imageWidth, int imageHeight,int angle) {
	vector < Rect > faces;
	Mat frame_gray;
	cvtColor(image, frame_gray, CV_BGR2GRAY);
	equalizeHist(frame_gray, frame_gray);

	mCascade1.detectMultiScale(frame_gray, faces, 1.1, 2, 0, Size(60, 60),Size(2147483647, 2147483647));
...
bool isMissTracking(TrackObj& trackObj, float left, float top, float right,
	...
	if (width * height > (trackObj.initSize * 1.5)) {
		...
		LOGE("too large missing tracking. %d %d", width * height,trackObj.initSize);
		return true;
	}
...
jfloatArray Java_com_smartcamera_core_CameraManager_nativeTrackingFace(
	...
	if (isTracking() && enableTracking && enableAsyncDetect) {
		...
		resize(bgrMat, bgrMat,Size(TRACKING_IMAGE_WIDTH, TRACKING_IMAGE_HEIGHT));

		int i;
		int effectiveFaceCount = 0;
		float *facesData = (float*) malloc(MAX_TRACKING_OBJ_SIZE * 4 * sizeof(float));
		for (i = 0; i < trackingCount; i++) {
			TrackObj &trackObj = trackObjs[i];
			if (trackObj.flag) {
				float *result = (float*) malloc(4 * sizeof(float));
				trackFace(trackObj, bgrMat, result, TRACKING_IMAGE_WIDTH,TRACKING_IMAGE_HEIGHT, env, obj);
				if (isMissTracking(trackObj, result[0], result[1], result[2],result[3])) {
...
jint Java_com_smartcamera_core_CameraManager_nativeEnableTracking(JNIEnv *env, jobject obj, jint isEnable) {
