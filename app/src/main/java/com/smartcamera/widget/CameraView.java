...
public class CameraView extends SurfaceView implements SurfaceHolder.Callback,
			...
			do {
				...
				if (!mStopThread && mCameraFrameReady) {
					...
					if(CameraManager.getInstance().nativeIsEnableAsyncDetect()==1){
						if (!isDetecting) {
							isDetecting = true;
							new DetectWorker(imageBytes, p.getPreviewSize().width, p.getPreviewSize().height).start();
						}
						
						Face[] faces = CameraManager.getInstance().trackingFace(imageBytes, p.getPreviewSize().width, p.getPreviewSize().height, -90);
						...
						Face[] faces = CameraManager.getInstance().findFaces(imageBytes, p.getPreviewSize().width, p.getPreviewSize().height, -90);
						if (faces != null && faces.length != 0) {
							drawFaces(faces);
						} else {
							drawFaces(null);
	...
	private class DetectWorker extends Thread {
		...
		@Override
		public void run() {
			CameraManager.getInstance().findFaces(imageBytes, width, height, -90);
			isDetecting = false;
		}
	};
}
